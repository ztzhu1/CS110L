...
fn read_file_lines(filename: &String) -> Result<Vec<String>, io::Error> {
    ...
    let ret = io::BufReader::new(file).lines().map(|x| x.unwrap()).collect();
    Ok(ret)
}
...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    for i in 0..grid.size().0 {
        grid.set(i, 0, 0).expect("Wrong set");
    }
    for j in 0..grid.size().1 {
        grid.set(0, j, 0).expect("Wrong set");
    }
    for i in 0..seq1.len() {
        for j in 0..seq2.len() {
            if seq1[i] == seq2[j] {
                grid.set(i + 1, j + 1, grid.get(i, j).unwrap() + 1).expect("Wrong set");
            } else {
                grid.set(
                    i + 1,
                    j + 1,
                    std::cmp::max(grid.get(i + 1, j).unwrap(), grid.get(i, j + 1).unwrap())).expect("Wrong set");
            }
        }
    }
    grid
