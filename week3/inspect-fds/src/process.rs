...
impl Process {
    ...
    pub fn list_fds(&self) -> Option<Vec<usize>> {
        let dir_str = format!("/proc/{}/fd", self.pid);
        ...
        for entry in fs::read_dir(dir).ok()? {
            ...
            let path = entry.path();
            if path.is_dir() {
                panic!("entry should not be a path!");
            } else {
                let path = path.into_os_string().into_string().unwrap();
                let id: Vec<&str> = path.split("/").collect();
                println!("{}", id.last().unwrap());
                v.push(
                    id.last()
                        .unwrap()
                        .to_string()
                        .parse()
                        .expect("fail to parse path as an usize!"),
                );
            }
        }
        Some(v)
